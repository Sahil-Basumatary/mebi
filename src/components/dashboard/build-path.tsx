...
export function BuildPath({ stages, className }: { stages: BuildStage[]; className?: string }) {
  return (
    <section
      aria-label="Build path"
      className={cn(
        "border-app-divider bg-app-divider grid gap-px border sm:grid-cols-2 2xl:grid-cols-4",
        className,
      )}
    >
      {stages.map((stage, index) => (
        <div key={stage.id} className="bg-app-paper flex flex-col p-3">
          <span aria-hidden className="bg-app-divider block h-1 w-full">
            <span className={cn("bg-app-accent block h-1", FILL_WIDTH[stage.state])} />
          </span>
